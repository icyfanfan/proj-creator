...
const { writeConfig , updateFileStructure } = require( './generator/fileOperator' )
...
module.exports = function custom( rootPath ){
    inquirer
        ...
        .then( answers => {
            ...
            let webpk = generator( { ...answers , rootPath } ) ,
            ...
            // 文件结构调整，修改package.json，调整入口文件index的类型（index.js/index.ts）
            updateFileStructure( { rootPath , ...answers } )
            // return console.log( prodConfig.toString() )
            // 写配置
